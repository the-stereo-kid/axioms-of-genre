...
export const useGenreStore = defineStore("genre", () => {
  ...
  async function selectGenre(genreId: number) {
    ...
    try {
      ...
      // .find() returns Genre | undefined
      let genre: Genre | null | undefined = genres.value.find((g) => g.id === genreId);

      // If not found, fetch from database
      // fetchGenreById returns Genre | null
      if (!genre) {
        genre = await fetchGenreById(genreId);
      }

      // Set to null if still not found (handles both null and undefined)
      selectedGenre.value = genre ?? null;

      // Load elements for this genre (only if we found it)
      if (genre) {
